...
for(const[label,href]of links){const link=document.createElement('a');link.className='dashboard-action fdn-nav-link';link.href=href;if(new URL(href,location.href).pathname===location.pathname)link.setAttribute('aria-current','page');link.textContent=label;nav.append(link);}
