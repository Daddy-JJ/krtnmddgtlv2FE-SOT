...
export function validateResumeDocx(file, { label = 'File' } = {}) {
  if (!file || typeof file.name !== 'string' || !Number.isFinite(file.size) || file.size <= 0) {
    return `Pilih ${label.toLowerCase()} .docx terlebih dahulu.`;
  }
  if (!file.name.toLowerCase().endsWith('.docx')) {
    return `${label} wajib menggunakan format .docx.`;
  }
...
export function validateResumeSourceDocx(file) {
  ...
  return validateResumeDocx(file, { label: 'CV sumber' });
}
